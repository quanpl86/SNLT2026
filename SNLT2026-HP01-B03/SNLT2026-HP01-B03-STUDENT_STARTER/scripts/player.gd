...
func _physics_process(delta: float) -> void:
	...
	# Xử lý Nhảy (Jump) - B03 GAP B: Cố ý thiếu is_on_floor() khiến nhân vật có thể nhảy trên không (air-jump)
	...
	# Gọi hàm lấy tốc độ di chuyển B03
	var move_speed := get_move_speed()

	if direction != Vector3.ZERO:
		velocity.x = direction.x * move_speed
		velocity.z = 0.0 # B03 GAP A: Cố ý gán 0.0 khiến phím W/S không tạo di chuyển theo trục Z
	else:
